            ...
            If Integer.TryParse(postedValues("eventId"), eventId) AndAlso Integer.TryParse(postedValues("gameId"), gameId) AndAlso Decimal.TryParse(postedValues("odd").Replace(".", ","), odd) Then
                If Single.TryParse(postedValues("betSum").Replace(".", ","), betSum) Then
                    ...
                    If rBetRestr.ErrorCode = 0 Then
                        ...
                        Else
                            status = "1"
                            message = "Ставка не сделана, сумма ставки не соответствует разрешенной для этой игры. Максимальная ставка: " + CInt(Math.Min(rBetRestr.MaxBet, rBetRestr.MaxGain / odd)).ToString() + " " + user.Currency.ToString() + ". Минимальная ставка: " + CInt(rBetRestr.MinBet).ToString() + " " + user.Currency.ToString() + ". "
                        End If
                    Else
                        status = "1"
